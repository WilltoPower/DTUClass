...
/**
 * Get the ASDU payload buffer left size
 *
 * The payload is the ASDU message part after the ASDU header (type ID, VSQ, COT, CASDU)
 *
 * \return the ASDU payload buffer left size
 */
int
CS101_ASDU_getPayloadLeftSize(CS101_ASDU self);

/**
 * Set the ASDU other pointer 
 * 
 * The 'ohter' pointer is a parameter which define by user, to transform some data
 * 
 * Note that, destory ASDU not free this pointer
 */
void
CS101_ASDU_setOther(CS101_ASDU self, void* parameter);

/**
 * Get the ASDU other pointer 
 * 
 * The 'ohter' pointer is a parameter which define by user, to transform some data
 */
void*
CS101_ASDU_getOther(CS101_ASDU self);
...
/**
 * \brief Create a 3 byte time from a UTC ms timestamp
 */
CP24Time2a
CP24Time2a_createFromMsTimestamp(CP24Time2a self, uint64_t timestamp);

/**
 * \brief Set the time value of a 3 byte time from a UTC ms timestamp
 */
void
