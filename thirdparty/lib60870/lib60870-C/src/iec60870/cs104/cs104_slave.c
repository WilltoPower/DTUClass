...
HighPriorityASDUQueue_enqueue(HighPriorityASDUQueue self, CS101_ASDU asdu)
{
    ...
    uint8_t* nextMsgPtr;
    DEBUG_PRINT("entryCounter:%i, buffer:%p, selfsize:%i\n", 
        self->entryCounter, self->buffer, self->size);
    ...
    if (self->entryCounter > 0) {
        if (nextMsgPtr <= self->firstEntry) {
            ...
            DEBUG_PRINT("ERROR, CS104 SLAVE: ASDUs in PRIO-FIFO: %i (new(size=%i/%i): %p, %p, first: %p, last: %p lastInBuf: %p)\n", 
                self->entryCounter, entrySize, asduSize, self->buffer, nextMsgPtr,
                self->firstEntry, self->lastEntry, self->lastInBufferEntry);
                enqueued = false;
            }
        }
...
static uint8_t STARTDT_INIT_END[] = { 0x68, 0x0E, 0x00, 0x00, 0x00, 0x00, 0x46, 0x01, 0x04, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00 };

#define STARTDT_INIT_END_SIZE 16
...
handleMessage(MasterConnection self, uint8_t* buffer, int msgSize)
{
    ...
    if (msgSize >= 3) {
        ...
        /* Check for STARTDT_ACT message */
            ...
            // 南网DTU摸底测试 发送结束标志
            // if (writeToSocket(self, STARTDT_INIT_END, STARTDT_INIT_END_SIZE) < 0)
            //     return false;

            sendASDU(self, STARTDT_INIT_END, STARTDT_INIT_END_SIZE, 0, NULL);
        }
...
sendNextHighPriorityASDU(MasterConnection self)
{
	...
	DEBUG_PRINT("Send Wait ASDU:%p",buffer);
    if (buffer) {
    ...
    if (self->oldestSentASDU != -1) {
        ...
        if (currentTime > self->sentASDUs[self->oldestSentASDU].sentTime) {
            ...
            if ((currentTime - self->sentASDUs[self->oldestSentASDU].sentTime) >= (uint64_t) (self->slave->conParameters.t1 * 1000)) {
                ...
                DEBUG_PRINT("CS104 SLAVE: I message timeout for %i seqNo: %i, current:%llu, oldest:%llu\n", self->oldestSentASDU,
                        self->sentASDUs[self->oldestSentASDU].seqNo, currentTime, self->sentASDUs[self->oldestSentASDU].sentTime);
            }
        }
    }
