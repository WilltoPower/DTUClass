...
void dturpcclient::init(const std::string& ip, uint16_t port, bool reinit)
{
	...
	_client.set_connect_timeout(RPC_TIMEOUT);
...
void dturpcclient::setInformationID(bool isrun)
{
	runInfomation = isrun;
}
...
void dturpcclient::run()
{
	std::thread t([&]() {
		while (1)
		{
			try
			{
				...
				for (;;)
				{
					...
					if (_client.has_connected() && execrun)
					{
						...
						if (InfoID == 0) {
							DTU::buffer sresult = _client.call<DTU::buffer>("rpc_read_parameter", PC_R_YC_DATA);
							emit update_information(1, QByteArray(sresult.const_data(), sresult.size()));
						}
					}
			...
			{
				DTULOG(DTU_ERROR, "查询信息发生错误:%s", e.what());
			}
		}
	});
	t.detach();
...
void dturpcclient::change_net_param()
{
	...
	this->init(cfg.ip, cfg.port, true);
}