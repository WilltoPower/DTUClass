...
void ProtoDialog::unpack(DTUCFG::DSYSCFG::StationCFG &cfg)
{
    if(prototype == 101) {
        ...
        ui->spin_cot->setValue(cfg.CS101.ALParam.sizeofCOT);
        ui->spin_ioa->setValue(cfg.CS101.ALParam.sizeofIOA);
        ui->check_zip->setChecked(cfg.CS101.EXParam.isSequence);
        // 链路层配置
        ui->check_char->setChecked(cfg.CS101.LLParam.SingalCharACK);
...
void ProtoDialog::check_zip_click(int state)
{
...
void ProtoDialog::check_char_click(int state)
{
	QCheckBox *check = (QCheckBox*)sender();
	if (state == Qt::Checked) {
