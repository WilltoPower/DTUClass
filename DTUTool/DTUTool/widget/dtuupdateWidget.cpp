...
void dtuupdateWidget::execute_update_program()
{
	if (!execute_test_arm_connect()) {
		ui.Ltxt->setText("升级失败,后台程序未连接");
		DTULOG(DTU_ERROR,"升级失败,后台程序未连接");
	}
...
bool dtuupdateWidget::translateFile(const std::string& src, const std::string destfile)
{
	try
	{
		...
		if (DTU_FAILED(execute_set_file_plus(src, execPath + destfile)))
		{
			return false;
		}
	}
	...
	{
		DTULOG(DTU_ERROR, "传送文件%s异常, %s", src.c_str(),e.what());
		return false;
	}
	return true;
}
