...
SerialCFG SerialDialog::pack()
{
    ...
    cfg.name = this->boardToSerial(ui->combobox_name->currentIndex());  // 串口名称
...
void SerialDialog::widgetChange()
{
	auto ObjectName = sender()->objectName();

	if (ObjectName == "combobox_name") {
	
	}
	else if (ObjectName == "combo_baud") {
	
	}
	else if (ObjectName == "spin_data") {
	
	}
	else if (ObjectName == "combo_pairty") {
	
	}
	else if (ObjectName == "spin_stop") {
