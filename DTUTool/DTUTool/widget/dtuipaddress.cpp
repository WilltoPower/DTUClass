...
IPAddress::IPAddress(QWidget *parent) : QWidget(parent)
{
	...
	qss.append(QString("QLabel{color:white;}"));
	qss.append(QString("QLineEdit{color:white;}"));
	qss.append(QString("QFrame{background-color:%1}").arg(bgColor));
...
void IPAddress::textChanged(const QString &text)
{
    ...
    //拼接成完整IP地址
    ip = QString("%1.%2.%3.%4").arg(txtIP1->text().toInt()).arg(txtIP2->text().toInt()).arg(txtIP3->text().toInt()).arg(txtIP4->text().toInt());
	emit IPChanged(ip);
}
...
void IPAddress::setEnabled(bool status) const
{
	txtIP1->setEnabled(status);
	txtIP2->setEnabled(status);
	txtIP3->setEnabled(status);
	txtIP4->setEnabled(status);
}
