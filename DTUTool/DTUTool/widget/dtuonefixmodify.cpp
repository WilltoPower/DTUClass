	...
	: QWidget(parent), ui(new Ui::dtuonefixmodifyClass())
{
	...
	ui->lineEdit_hex->setText("0x" + QString("%1").arg(fixid, 4, 16, QLatin1Char('0')).toUpper());
...
void dtuonefixmodify::checkid()
{
	...
	if (!execute_test_arm_connect()) {
		QMessageBox::information(this, "检验点表", "未连接后台程序",
			QMessageBox::Ok);
		return;
	}
...
void dtuonefixmodify::modifyfixid()
{
	...
	if (QMessageBox::Ok == QMessageBox::question(this, "确认修改点表", "是否确认修改点表",
