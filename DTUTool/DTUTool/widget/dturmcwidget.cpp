	...
	, ui(new Ui::dturmcwidgetClass())
{
	ui->setupUi(this);
	//load_ui();
	connect(ui->btn_lock, SIGNAL(clicked()), this, SLOT(checkpassword()));
}
...
void dturmcwidget::load_ui()
{
	...
	{
		ui->tableWidget->setCellWidget(index, 0, QCreateHelper::createStrLabel(QString::number(index + 1)));
		ui->tableWidget->setCellWidget(index, 1, QCreateHelper::createStrLabel("0x" + QString("%1").arg(item.second.fixid, 2, 16, QLatin1Char('0')).toUpper()));
		ui->tableWidget->setCellWidget(index, 2, QCreateHelper::createStrLabel(QString::fromStdString(item.second.desc)));
		auto rmcwidget = QCreateHelper::createRmcWidget(item.second.fixid);
...
void dturmcwidget::rmctrlhanlder(int opt, uint16_t fixid)
{
	if (execute_test_arm_connect())
	{
		...
		if (ret != DTU_SUCCESS)
			DTULOG(DTU_ERROR, "下发遥控命令错误");
	}
}

void dturmcwidget::checkpassword()
{
	if (ui->edit_user->text() == "sduept" && ui->edit_pass->text() == "1000") {
