...
dtuCalibrateTimeDlg::~dtuCalibrateTimeDlg()
{
	//在退出窗口之前，实现希望做的操作
	_run_query = false;
	if (_query_local_thread)
	{
		_query_local_thread->join();
	}
	if (_query_device_thread)
	{
		_query_device_thread->join();
	}
	if (_time_skew)
	{
		_time_skew->join();
...
void dtuCalibrateTimeDlg::query_device_time()
{
	while (_run_query)
	{
		...
		{
			QtDTULOG(DTU_ERROR, "query_device_time() %s", e.what());
		}
...
void dtuCalibrateTimeDlg::Manual_Calibrate_Time()
{
	...
	if (DTU_SUCCESS != execute_write_data(PC_W_CLK, data))
	{
		QtDTULOG(DTU_ERROR, "手动写入时间参数错误");
		return;
	}
	QtDTULOG(DTU_INFO, "手动写入时间参数");
}