...
void dtureport::ReportParseRMC(QTableWidget *widget, DTU::buffer &data, int index, int no)
{
	...
	switch (co)
	{
	case 0: coStr = "分闸"; break;
	case 1: coStr = "合闸"; break;
	default:
...
void dtureport::LoadReportFile(int row, int clcolumn)
{
	...
	case ReportPro: {
		...
		if (ret) {
			...
			if (ft.open(QIODevice::ReadOnly)) {
				...
				QtDTULOG(DTU_ERROR, (char*)"无法打开文件:%s", fileName.toStdString().c_str());
			}
			if (protact.size() > 0) {
			...
			QtDTULOG(DTU_ERROR, "无法获取保护录波文件%s", fileName.toStdString().c_str());
		}
	}; break;
	// 保护录波档案
	case ReportTransRcd: {
	...
	case ReportWorkRcd: {
		...
		if (ret) {
			...
			QtDTULOG(DTU_ERROR,"无法获取业务录波文件%s", fileName.toStdString().c_str());
		}
	}; break;
	default:
		break;
...
void dtureport::open_comtrade_file(QString fileName)
{
	...
	std::string fdranalyse = dtusysconfig::instance().get_analyze_tools();

	std::string cmdLine = "start " + fileName.toStdString();
	//用于部分入参CString向LPCWSTR转换
	USES_CONVERSION;

	if (!CreateProcess((fdranalyse.c_str()),
		const_cast<char*>(cmdLine.c_str()), NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi))
	{
		QtDTULOG(DTU_ERROR, "无法启动离线分析工具");
		return;
	}
