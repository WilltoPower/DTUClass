...
void dtufixmodify::load_ui()
{
	...
	for (const auto &item : rmtable)
	{
		...
		// 映射点表
		tableSOE->setItem(index, 1, new QTableWidgetItem("0x" + QString("%1").arg(item.second.fixno, 4, 16, QLatin1Char('0')).toUpper()));
	...
	// 2.创建遥测点表
	auto rctable = DBManager::instance().GetInfomationTableByIndex(InfomTelemetry);
	header.clear();
	header << "序号" << "映射点表值" << "分组" << "描述";

	QTableWidget *tableCOS = nullptr;

	createTabWidget(tableCOS, rctable.info.size(), header);
	
	index = 0;
	for (const auto &item : rctable.info)
	{
		// 序号
		...
		// 映射点表
		tableCOS->setItem(index, 1, new QTableWidgetItem("0x" + QString("%1").arg(item.second.fixid, 4, 16, QLatin1Char('0')).toUpper()));
	...
	for (const auto &item : rmctable)
	{
		...
		// 映射点表
		tableRMC->setItem(index, 1, new QTableWidgetItem("0x" + QString("%1").arg(item.second.addr, 4, 16, QLatin1Char('0')).toUpper()));
	...
	{
		if (item.second.pid == ParamCommunication ||
	...
	for (const auto &item : rptable)
	{
		...
		for (const auto &item1 : item.second.info)
		{
			...
			// 映射点表
			tableRP->setItem(index, 1, new QTableWidgetItem("0x" + QString("%1").arg(item1.second.fixid, 4, 16, QLatin1Char('0')).toUpper()));
