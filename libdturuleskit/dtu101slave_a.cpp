...
// 数据处理
bool D101SlaveSecond::dtu_asduHandler(IMasterConnection connection, CS101_ASDU asdu)
{
   auto type = CS101_ASDU_getTypeID(asdu);
    DTULOG(DTU_INFO, (char *)"主站B 接收到101请求:%u", type);

    dtuUnitRPCTranser trans;
    trans.setCallback(std::bind(&D101SlaveSecond::send_asdu, this, std::placeholders::_1, std::placeholders::_2), connection);
...
// 链路状态改变
void D101SlaveSecond::dtu_linkLayerStateChanged(int address, LinkLayerState state)
{
    char timeBuffer[32];
    time_t tt;
    struct tm *ttime;
    ...
    // 发送消息
    connectionMessage(CSLOG,true,102,log.size(),nullptr,log);
    ...
    if(state == LL_STATE_ERROR) {
        ...
        CS101_ASDU asdu;
        asdu = CS101_ASDU_create(this->get_applayer_param(), false, CS101_COT_INITIALIZED, 
        DSYSCFG::instance().ASDU(), DSYSCFG::instance().ASDU(), false, false);
        CS101_ASDU_setTypeID(asdu, M_EI_NA_1);
        CS101_ASDU_setNumberOfElements(asdu, 1);

        DTU::buffer payload(this->get_applayer_param()->sizeOfIOA);
        uint8_t QRP = 0x00;
        payload.append((char*)&QRP, sizeof(uint8_t));
        CS101_ASDU_addPayload(asdu, (uint8_t*)(payload.const_data()), payload.size());
        this->send_asdu(asdu, CS101_Slave_getIMasterConnection(this->_slave));
    }
...
void D101SlaveSecond::dtu_rawMessageHandler(uint8_t* msg, int msgSize, bool sent)
{
    connectionMessage(CSMSG,sent,102,msgSize,msg,std::string(""));
...
// 时钟同步
bool D101SlaveSecond::dtu_clockSyncHandler (IMasterConnection connection, CS101_ASDU asdu, CP56Time2a newTime)
{
    // 下发时间
...
// 总召唤
bool D101SlaveSecond::dtu_interrogationHandler(IMasterConnection connection, CS101_ASDU asdu, uint8_t qoi)
{
    // 只相应主站召唤
    if (qoi == 20) {
        DTULOG(DTU_INFO, (char *)"主站B 101总召唤");
        // 发送总召确认
...
// 发送ASDU数据
void D101SlaveSecond::send_asdu(CS101_ASDU asdu, IMasterConnection con)
{
    if (asdu){
        if (con){
...
clockSyncHandler (void* parameter, IMasterConnection connection, CS101_ASDU asdu, CP56Time2a newTime)
{
    if (parameter)
    {
        return ((D101SlaveSecond*)parameter)->dtu_clockSyncHandler(connection, asdu, newTime);
    }
    return false;
}
...
interrogationHandler(void* parameter, IMasterConnection connection, CS101_ASDU asdu, uint8_t qoi)
{
    if (parameter)
    {
        return ((D101SlaveSecond*)parameter)->dtu_interrogationHandler(connection, asdu, qoi);
    }
    return false;
}
...
asduHandler(void* parameter, IMasterConnection connection, CS101_ASDU asdu)
{
    if (parameter)
    {
        return ((D101SlaveSecond*)parameter)->dtu_asduHandler(connection, asdu);
    }
    return false;
}
...
linkLayerStateChanged(void* parameter, int address, LinkLayerState state)
{
    if (parameter)
    {
        ((D101SlaveSecond*)parameter)->dtu_linkLayerStateChanged(address, state);
    }
}
...
rawMessageHandler (void* parameter, uint8_t* msg, int msgSize, bool sent)
{
    if (parameter)
    {
        ((D101SlaveSecond*)parameter)->dtu_rawMessageHandler(msg, msgSize, sent);
    }
}
...
void D101SlaveSecond::dtu101_run_slave()
{
    //
...
void D101SlaveSecond::dtu101_stop_slave()
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 获取AP参数
CS101_AppLayerParameters D101SlaveSecond::get_applayer_param()
{
    return CS101_Slave_getAppLayerParameters(_slave);
}

// 读取地址
int32_t D101SlaveSecond::get_address()
{
    return DSYSCFG::instance().ASDU();
}

void D101SlaveSecond::notify_master(CS101_ASDU asdu, bool autodestroy)
{
    // asdu如果不为空则发送,否则不发送
    if (asdu == nullptr)
        return;
